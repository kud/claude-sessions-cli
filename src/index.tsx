...
const readFirstPrompt = (filePath: string): string => {
  try {
    return parseFirstPromptFromJsonl(readFileSync(filePath, "utf8"))
  } catch {
    return ""
  }
}
...
const readSessionTitle = (filePath: string): string => {
  try {
    return parseSessionTitleFromJsonl(readFileSync(filePath, "utf8"))
  } catch {
    return ""
  }
}
...
const analyzeSessionMove = (
  ...
  try {
    ...
    return analyzeSessionMoveContent(readFileSync(srcPath, "utf8"), fromDir)
  } catch {
...
// The cli-shot contract: --screen opens directly on a tab and --screen list
// enumerates them, so screenshot tooling can shoot each one without carrying a
// table of which keystrokes reach which tab. Read from argv rather than a
// parser because this entry has none.
//
// Safe to set the tab alone here: the effects that load a tab's data are keyed
// on [tab], so they fire on mount with whatever it starts as. A CLI that loads
// from its tab-switch handler instead needs that handler called, or it opens on
// a tab that never fetched.
const SCREENS: Tab[] = ["code", "chat", "schedule"]
const screenAt = process.argv.indexOf("--screen")

if (screenAt !== -1) {
  const requested = process.argv[screenAt + 1]
  if (requested === "list") {
    console.log(SCREENS.join("\n"))
    process.exit(0)
  }
  if (!SCREENS.includes(requested as Tab)) {
    console.error(
      `Unknown screen "${requested ?? ""}". Available: ${SCREENS.join(", ")}`,
    )
    process.exit(1)
  }
  savedState = { ...savedState, tab: requested as Tab }
}
